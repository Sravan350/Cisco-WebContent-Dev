	...
	var clientAppRevokeMsg = '<p class="integrationAuthMsg"><span class="info-icon" title=""> &nbsp;</span><span>We are facing difficulty while accessing your Box account. Please click on Add to authorize.</span></p>';
    ...
    $( document ).ready(function() {
		...
		$(document).on("click", "#instance-list-block .remove-config" , function() {
			...
			$("#remove-msg").html("Your configured room is not available. Are you sure you  want to remove this configuration?");
			else
	...
	function getRoomsInfo(){
		$.ajax({
			...
			success: function(result){
				...
				if(result["integrationSpecificSettings"]["validToken"]=="false" && result["integrationSpecificSettings"]["validToken"] != undefined) isClientAppTokenValid = false;
				else isClientAppTokenValid = true;
	...
	function setUpRooms(){
		console.log("Loading all rooms len : "+allRooms.length);
		if(allRooms.length !=0){
    ...
    function validateClientAppToken() {
		console.log("validateClientAppToken called in validateTokenDB");
		$.ajax({
			url: configuration_settings_url+"?userId="+user_id,
			async: true,
			success: function(result){
				if(result["integrationSpecificSettings"]["validToken"]=="false" && result["integrationSpecificSettings"]["validToken"] != undefined){
					isClientAppTokenValid = false;
					$('.client-app-warning').html(clientAppRevokeMsg);
					$('.edit-config').css('z-index', '-1');
					$('.cross-mark').css('z-index', '-1');
	...
	console.log("_____ validateIntegrationAppTokenDB() ________Start");	
		$.ajax({url: get_app_token_url+"?userId="+user_id,
			...
			success: function(result){
				if(result.tokenResult.tokenId) {
					...
					console.log(" isClientAppTokenValid after validateClientAppToken() " + isClientAppTokenValid);
	...
	function removeIntegrationInstance(instanceId,removeRoomId) {
		...
		if(isRoomAvailable>1){
			...
			removeMessageToSpark = appName+" has been removed from this room";
		}
