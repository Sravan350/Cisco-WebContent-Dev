    ...
    $( document ).ready(function() {
		...
		$(document).on("click", "#instance-list-block .remove-config" , function() {
			...
			console.log("globleinstance in remove "+JSON.stringify(globleinstance)+" "+ globleinstance[0].statusCode+" "+instanceId  );
			//globleinstance['spaceDetailsResponse'];
			$.each(globleinstance,function(id,roomData){
				if(roomData.id != ""){
					if(instanceData.channelId == roomData.id){
						console.log("&&&&&& "+roomData.title);
						selectedRoomName = roomData.title;
					}
	...
	function setUpRooms(){
		if(allRooms.length !=0){
			...
			$("#rooms").html('<option value="0">Please Select</option>');
