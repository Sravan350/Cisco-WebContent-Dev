    ...
    function resize(){ 
		...
		//var frameId = document.getElementById("guestFrame1");
		//var frameId = iframe.contentWindow.document.body.scrollHeight;
		//var scH = frameId.scrollHeight;
	...
	function listInstancesData(result, checkAllRooms) {
		...
		$.each(listResultData, function(i, instance) {
			...
			if(roomname == "No")
			{
				...
				listDataHTML+='<label class="room">'+roomname+' Space</label>';
	...
	function sortRooms(){
		 ...
		 console.log("Sorted data in sortRooms() : "+JSON.stringify(allRooms));
	}
			...
			$.ajax({
				...
				success: function(result){
					...
					if(isClientAppTokenValid== false){
						$('.edit-remove-icons').css("z-index","-1");
					}
				},
				error: function(xmlHttpRequest,error) {
				   if(xmlHttpRequest.readyState == 0 || xmlHttpRequest.status == 0) {
