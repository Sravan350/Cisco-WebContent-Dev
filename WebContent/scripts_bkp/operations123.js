	...
	function noListMessage(){
		var noListDataHTML='<hr class="instance-hr"><div class="large-12 medium-12 columns instance-background">';
			...
			/*else if(application_id=='1')
				 noListDataHTML+='<div class="no-config-msg">You have no configurations. Click on “Add Bitbucket Server Configuration” or “Add Bitbucket Cloud Configuration” above to set one up.</div></div>';
			 */
			 else if(twowayFlag){
				 console.log("************************");
				noListDataHTML+='<div class="no-config-msg">You have no configurations. Click on one of above buttons to set up one.</div></div>'; 
			 }else {console.log("&&&&&&&&&&&&&&&&&&&&");
	...
	function setUpRooms(){
		if(allRooms.length !=0){
			...
			selected_room_id = 0; // commented for trello rooms setting
					...
					if(authenticationFlag){
						if(appTokenExists){
						   ...
						   // $(".bitbucket-addconfig").html('');
							$(".bitbucket-addconfig").html('Add Bitbucket Cloud  Configuration');
						...
						$(".bitbucket-addconfig").html('');
						$(".bitbucket-addconfig").html('Add Bitbucket Server Configuration');
						$(".github-add-config").show();
						$(".bitbucket-add-config").show();
					}
					if(!isClientAppTokenValid){
			...
			success: function(result) {
				if(result.tokenResult.tokenId) {
					...
					if(application_id=="3" || application_id=="1"){
						...
						authReqMsg+='<div class="no-config-msg">You are not authorized to this integration. Click one of the above button to continue.</div></div>';
						$('#integrations-block').show();
	...
	function removeIntegrationInstance(instanceId,removeRoomId) {
		var twowayRoomCount = 0;
		var oneWayRoomCount = 0;
		var c = 0;
		$.each(listResultData,function(index,value){
			if(value["instanceId"]==instanceId) { 
				var configJ = JSON.parse(value['configJson']);
				console.log("twoway ::::: --- > "+JSON.stringify(configJ)+" value ::: "+JSON.stringify(value));
				if(value["channelId"]==removeRoomId && configJ["twoway"] == true) { 
					c++;
					twowayRoomCount++;	
				}else if(value["channelId"]==removeRoomId){
					oneWayRoomCount++;
				}
				console.log("C :: "+c);
			}
		});
		console.log("oneWayRoomCount :::: "+oneWayRoomCount+" twowayRoomCount:::: "+twowayRoomCount);
		if(twowayFlag){
			
		}else{
			
		}
		if(oneWayRoomCount>1){
		...
		//if(configJ['twoway']!=undefined && configJ['twoway']){
			...
			$.each(listResultData,function(index,value){
				if(value["instanceId"]==instanceId) { 
					...
					if(configJ['twoway']!=undefined && configJ['twoway'] && !flagTwoWay){
						removeMessageToSpark = "Hi <@personId:"+user_id+">,<br>Your Trello Configuration has been disabled for this space. ";
			...
			if(sameroomcount>1 && flagTwoWay)
				deleteUrl = delete_instance_url+instanceId+"?messageToSpark=&isBot=true";
		//}
