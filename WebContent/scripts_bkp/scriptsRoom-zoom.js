	...
	//if (/Mobi/.test(navigator.userAgent)) {isMobile=true;alert("mobile device");}
    ...
    $( document ).ready(function() {
		if((navigator.userAgent.match(/iPhone/i)) || (navigator.userAgent.match(/iPod/i))) {
			alert("apple!!!!!");
		/*	$('.custom-select').css("font-size","50px !important");
			$('.custom-select span').css({position: 'absolute'});
			$('.custom-select span').css("margin-top","-11px");
			$('.custom-select a').css({position: 'absolute'});
			$('.custom-select a').css("margin-top","9px");*/
		}
		...
		$(document).on("click", ".custom-select-open a" , function() {
			//alert("clicked...");
		});
	...
	function listInstancesData(result, checkAllRooms) {
		...
		console.log("listResultData.length!!!!!!!!!!!" + listResultData.length);
		...
		listCheckAllRooms = false;
				...
				if(application_id=="3"){
					if(authenticationFlag){
						console.log("loadAllInstancesData!!! authenticationFlag!!!!!" + authenticationFlag);
						if(appTokenExists){
							$(".unauth").show();
					...
					if(!isClientAppTokenValid){
						console.log("loadAllInstancesData!!! isClientAppTokenValid!!!!!" + isClientAppTokenValid);
						$('.auth').hide();
	...
	function validateClientAppToken() {
		console.log("in validateClientAppToken!!!!!!!!!!!!");
		$.ajax({
			url: configuration_settings_url+"?userId="+user_id,
			async: true,
			success: function(result){
				//console.log("in validateClientAppToken success!!!!!!!!!!!!" + JSON.stringify(result));
	...
	function validateIntegrationAppTokenDB() {
		console.log("in validateIntegrationAppTokenDB!!!!!!!!!!!!");
		$.ajax({url: get_app_token_url+"?userId="+user_id,
			...
			success: function(result) {
				if(result.tokenResult.tokenId) {
					...
					if(application_id=="3"){		
						console.log("in github condition !!!!!!!!!!!!!!!!!!!!!!!!!");
						listIntegrations();
					}else{
						console.log("in else not github condition !!!!!!!!!!!!!!!!!!!!!!!!!");
						var authReqMsg ="";
