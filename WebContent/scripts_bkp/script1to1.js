    ...
    $( document ).ready(function() {
		...
		$(document).on("click", "#instance-list-block .remove-config" , function() {
				...
				if(allRooms.length!=0){
					$.each(allRooms, function(id,obj){
						if(instanceData.channelId==obj.id){
							...
							removeRoomId = instanceData.channelId;
							selectedRoomName = "private_message";
						}		
					});
				}else{
			...
			else if(selectedRoomName == "private_message")
				$("#remove-msg").html("Are you sure you want to remove this Private message (only to me) configuration?");
			else
	...
	function listInstancesData(result, checkAllRooms) {
		...
		$.each(listResultData, function(i, instance) {
			...
			var privateMessage = configData.private_message;
			console.log("list prepartion ****"+privateMessage);
			if(privateMessage=="true" && privateMessage!=undefined){
						...
						listDataHTML+='<label class="room">'+"Private Message to Me"+'</label>';
	...
	function loadRooms(){
	 ...
	 function setUpRooms(){
		console.log("All Rooms len : "+allRooms.length);
		if(allRooms.length !=0){
			...
			console.log("selected_room_id in setupRooms "+selected_room_id);
			$("#rooms").val(selected_room_id);
			$('.custom-select span').html($("#rooms option:selected").text());
			selected_room_id = 0;
			$("#rooms").customselect();
			console.log("isPrivate_config in loading all rooms ..."+isPrivate_config);
			if(isPrivate_config){
				$("#rooms").val(0);
				$('.custom-select span').html($("#rooms option:selected").text());
