	...
	function listInstancesData(result, checkAllRooms) {
		...
		$.each(listResultData, function(i, instance) {
			...
			console.log("configData..." + configData.unauthmode + " isClientAppTokenValid.." + isClientAppTokenValid);
			if(!checkAllRooms){	
				if(rooms.length!=0){
					$.each(rooms, function(id,obj){
			...
			if(roomname == "No")
			{
				...
				listDataHTML+='</div>';
				if(!checkAllRooms && roomname == "No"){	
					if(!configData.unauthmode&&!isClientAppTokenValid&&configData.unauthmode!=undefined&&configData.unauthmode!=""&&configData.unauthmode!=null){
	...
	function getRoomsInfo(){
		$.ajax({
			...
			success: function(result){
				...
				if(result["integrationSpecificSettings"]["isValidToken"]==false && result["integrationSpecificSettings"]["isValidToken"] != undefined) {isClientAppTokenValid = false;}
	...
	if(allInstancesResponse && roomsEventResponse) {
		...
		listCheckAllRooms = false;
				...
				$('#instance-loading-block').hide();
				console.log("in loadAll instances Data");
		...
		$.ajax({
			...
			success: function(result){
				if(result["integrationSpecificSettings"]["isValidToken"]==false && result["integrationSpecificSettings"]["isValidToken"] != undefined){
					...
					console.log("***** In validateClientAppToken "+isClientAppTokenValid);
