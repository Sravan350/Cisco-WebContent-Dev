	...
	function listInstancesData(result) { 
		var pollingtim = window.setInterval(function(){
			if(allRooms.length > 0 ){
				window.clearInterval(pollingtim);
				var listDataHTML="";
				var noRoomNameList =[];
				listResultData=result;
				if(listResultData.length>0){
					if(arraybysantosh == ""){
						$.each(allRooms, function(id,obj){
							arraybysantosh+=obj.id+",";
							arraybysantosh1+=obj.title+",";
						});
					arraybysantosh = arraybysantosh.split(",");
					arraybysantosh1 = arraybysantosh1.split(",");
					}
			//console.log("arraybysantosh1 "+arraybysantosh1)
			//console.log("arraybysantosh "+arraybysantosh)
		$.each(listResultData, function(i, instance) {
			var roomname="No";
			var configData=JSON.parse(instance.configJson);
			/*if(!checkAllRooms){
			...
			if(listCheckAllRooms == false){
				...
				if(allRooms.length!=0){
					...
					console.log("================================="+instance.channelId)
					arraybysantosh = arraybysantosh.split(",");
					*/
					
					console.log("********************* room matched "+instance.channelId+"2222222222222222222222"+arraybysantosh[0]+" ---> "+arraybysantosh.indexOf(instance.channelId))
					if(arraybysantosh.indexOf(instance.channelId) > -1){
						var index = arraybysantosh.indexOf(instance.channelId);
						roomname = arraybysantosh1[index];
			...
			/*if(roomname == "No")
			{
				...
				listDataHTML+='<img src="'+integration_app_icon_url+'" alt="">';
				listDataHTML+='<label>'+configData.displayName+'</label>';
				if(roomname == undefined || roomname=="No"){
						console.log("No room found .... ")
						listDataHTML+='<label class="room"></label>';
				}else{
						console.log("Room found ...."+roomname);
	...
	function sortSpaces(){
		allSpaces = allSpaces .sort((function (a, b) { return new Date(b.lastActivity) - new Date(a.lastActivity) }));
	}
	...
	function getAllSparkSpaces(){
		console.log("getAllSparkSpaces calledd..");
		$.ajax({
			url: getSparkRooms+"?userId="+user_id,
			async: true,
			success: function(result){
				spaces_list = result;
				if(result["sparkRoomSettings"]!=undefined || result["sparkRoomSettings"]!=null){
					allSpaces = result["sparkRoomSettings"]["items"];
					sortSpaces();
					allRooms = allSpaces;
				}
				//$(".rooms-loading").hide();
	...
	function loadSpaces(){
		var spacesTimer = setInterval(function(){
			if(spaces_list != ""){
				clearInterval(spacesTimer);
				setUpSpaces();
			}
		},100);
	}
	function setUpRooms(){
		if(allRooms.length !=0){
			...
			console.log("selected_room_id from scripts Rooms "+selected_room_id);
	...
	function setUpSpaces(){
		console.log("*** Setup spacess to drop down ")
		if(allSpaces.length !=0){
			$("#rooms").find("option:gt(0)").remove();
			//$('#rooms option[value!="0"]').remove();
			$.each(allSpaces, function(id,obj){
				console.log("................");
				$("#rooms").append($('<option>').text(obj.title).attr('value',obj.id));
				if(formAction == "update" && selected_room_id1==obj.id && roomFlag==false) {selected_room_id = obj.id; roomFlag=true;}
			});
			console.log("roomFlag value " +roomFlag)
			if(formAction=="update" && roomFlag==false){
				console.log("No room found in all rooms")
				selected_room_id = "0";
	...
	function listIntegrations() {
		...
		getAllListInstances();
		/*if(rooms.length == 0){         //0704
			roomsEventResponse =false;
			getRoomsInfo();
		}*/
		console.log("Rooms length "+allRooms.length);
		getAllSparkRooms();
		roomsEventResponse =true; // 0704
		 /*if(allRooms.length == 0){
		  getAllSparkRooms();
