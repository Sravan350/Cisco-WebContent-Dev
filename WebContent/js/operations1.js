	...
	var application_id = "";
	ciscoUsersPermissionMsg = '<label style="color:#727272; margin-top: 2px; font-size: 0.775rem;">Disabled for Cisco</label>';
	...
	bannerMsg = 'Database upgrade is in progress so you may have some issues using this integration.';
...
//ciscoUsersPermissionMsg = '<lable>Disabled for Cisco</lable>';
	var _isValidSparkScope= false; //trello 2way
	var csrfJwt = ""; // csrf token value 
	var isCiscoUser = "";
	...
	//	$(".warning-msg").show();
		...
		isCiscoUser = gup(url, "isCiscoUser"); // Checking user is cisco user or not
		...
		$(document).on("click", "#instance-list-block .remove-config, #unauth-instance-list-block .remove-config" , function() {
			//if(isMobile)
			instanceId=this.id;
			$(".spark_scope_alert").css("display","none"); // trello 2way
		...
		$('#consumercopy1').on('click',function(){
			...
			setTimeout(function()
			{
			$('#consumercopy1').text("Copy Consumer Key");
			},1500) 
		});	
		...
		$('#publiccopy').on('click',function(){
			...
			setTimeout(function()
			{
			$('#publiccopy').text("Copy Public Key");
			},1500) 
		});
	...
	function checkValidSparkScopes(){
		if(listResultData.length>0){
			$.each(listResultData, function(i, instance) {
				var _instanceData=JSON.parse(instance.configJson);
				console.log(_instanceData.hasOwnProperty("twoway")+"  "+_instanceData.twoway);
				if(_instanceData.hasOwnProperty("twoway")){
					console.log("_instanceData.twoway "+_instanceData.twoway)
					_isValidSparkScope = true;
					return false;
				}
			});
		}else{
...
/*++++++++++++++++++++ Auth instances and unauth instances ++++++++++++++++++++++ */	
/*	function listInstancesData(result){
		console.log("in listInstancesData!!!!!!!!!!!");
		var listDataHTML = "";
	...
	function noListMessage(){
		...
		if(application_id=='1') noListDataHTML+='<div class="no-config-msg"><label>You have no configurations. Click on one of the above buttons to set one up.</label></div></div>';
		else noListDataHTML+='<div class="no-config-msg"><label>You have no configurations. Click Add to set up one.</label></div></div>';
					...
					/*if(result['sparkPersonDetails']!=undefined && application_id==7 || application_id==1 || application_id==3){
						if(result['sparkPersonDetails']['emails']!=undefined && result['sparkPersonDetails']['emails']!=null){
							...
							for(var i=0;i<emails.length;i++){
								if(emails[i].indexOf('@cisco.com')>=0){
									emailfalg=true;
									break;
								}
							}
							if(emailfalg){
								...
								$('a.ciscousrhide').on('click', function(e) { e.preventDefault(); });
									$(".secondary.button").css("color", "white");
									$(".secondary.button").css("opacity", "1");
								    $(".secondary.button").css("background", "#049fd9;");
					
								   $('.cisco_users_permission_error').html(ciscoUsersPermissionMsg);
								   $('.cisco_users_permission_error').show();
								  
								$('.ciscoAuthLinkDiv').append('<div style="position:absolute; left:0; right:0; top:0; bottom:0;" class="ciscousr-auth-link-block"></div>');
	...
	function setUpRooms(){
		if(allRooms.length !=0){
			...
			$('#select2-rooms-container').attr("title",$("#rooms option:selected").text()); // new
			selected_room_id = 0; 
	...
	function listIntegrations() {
		...
		if(isCiscoUser=="true" && application_id==7 || application_id==1 || application_id==3){
			$(".ciscousrhide").prop('disabled', true);
			$('a.ciscousrhide').on('click', function(e) { e.preventDefault(); });
			$(".secondary.button").css("color", "white");
			$(".secondary.button").css("opacity", "1");
	...
	if(allInstancesResponse) {
		...
		listCheckAllRooms = false;
				...
				$(".home").show(); 
				if(application_id=="1" || application_id=="3"){
